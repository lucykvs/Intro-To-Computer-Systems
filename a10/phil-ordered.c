...
void *phil_thread(void *arg) {
  ...
  while (meals < num_meals) {
    ...
    deep_thoughts();
    // pick up lowest numbered fork
    if (id < ((id + 1) % num_phils)) {
      getfork(leftfork(id));
      VERBOSE_PRINT("lowest numbered fork (left fork) picked up by phil %d\n", id);
    }
    else {
      getfork(rightfork(id));
      VERBOSE_PRINT("lowest numbered fork (right fork) picked up by phil %d\n", id);
    }
    
    // think after getting first fork
    deep_thoughts();

    // pick up second fork
    if (id < ((id + 1) % num_phils)) {
      getfork(rightfork(id));
      VERBOSE_PRINT("second fork (right fork) picked up by phil %d\n", id);
    }
    else {
      getfork(leftfork(id));
      VERBOSE_PRINT("second fork (left fork) picked up by phil %d\n", id);
    }
